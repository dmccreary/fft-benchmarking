...
- The **CMSIS-DSP library** — often still called by its older name, the **Arm Math
  ...
  like the Cortex-M33. "Hand-optimized" means something different per core and per data
  type: its Q15/Q31 fixed-point kernels use the DSP extension's packed SIMD instructions,
  its Cortex-M55/M85 kernels use Helium vectors, and its `arm_cfft_f32` float routine on
  an M33 is scalar code made fast by unrolling and structure. Chapter 22 covers why that
  distinction matters when you read performance claims.
